...
writeClauses :-
        ...
        minSizeTeam,
        maxSizeTeam,
        noIncompatibleWorkersTogether,
        true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachWorkerInOneTeam :-
        worker(W),
        findall(wt(W,T), team(T), Lits), exactly(1,Lits),fail.
eachWorkerInOneTeam.

minSizeTeam:-
        team(T),
        findall( wt(W, T), worker(W), L), minSize(MinSize), atLeast(MinSize, L), fail.
minSizeTeam.

maxSizeTeam:-
        team(T),
        findall( wt(W, T), worker(W), L), maxSize(MaxSize), atMost(MaxSize, L), fail.
maxSizeTeam.

noIncompatibleWorkersTogether :-
        incompatibleWorkers(W1, W2), team(T), 
        writeOneClause([ -wt(W1, T), -wt(W2, T)]), fail.
