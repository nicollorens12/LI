...
writeClauses(MaxContainers) :-
    eachBarOneContainer(MaxContainers),      % each bar goes to exactly one container
    barsDontExceedLength(MaxContainers),     % total length of bars in each container <= MaxL
    relateVariables(MaxContainers),          % relate bc(I,J) with c(J)
    containersAreConsecutive(MaxContainers), % containers are numbered 1, ..., MaxContainers
    true,!.
writeClauses(_) :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachBarOneContainer(MaxContainers) :-
    bar(I), findall(bc(I,J),between(1,MaxContainers,J),Lits),
    exactly(1,Lits), fail.
eachBarOneContainer(_).

% generateSubset(K1,K2,S,Sm): Sm is a subset of S with sum s.t. K1 < S <= K2
generateSubset(K1, K2, [E|_], [E]) :- E > K1, E =< K2.
generateSubset(K1, K2, [X|S1], [X|S2]) :- 
    K1P is K1 - X, K2P is K2 - X,
    generateSubset(K1P, K2P, S1, S2).
generateSubset(K1, K2, [_|S1], S2) :- 
    generateSubset(K1, K2, S1, S2).
...
minSubset(K1, L, Sm) :-
    max_list(L,Max), K2 is K1 + Max,
    generateSubset(K1, K2, L, Sm),
    sum_list(Sm, S),
    min_list(Sm, Min), S - Min =< K1.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% A less efficient version of minSubset/3:
%% 
%% subset([], []).
...
costOfThisSolution(M,Cost) :- findall(J,member(bc(_,J),M),L), max_list(L,Cost), !.
