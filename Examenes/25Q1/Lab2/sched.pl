...
insideMaximumMakespan([S|Sr],[D|Dr],Limit) :-
    S + D #=< Limit,
    insideMaximumMakespan(Sr,Dr,Limit).
    
% [1-2,2-3]
precedenceConstraints(_, _, []).
precedenceConstraints(S,D,[T1-T2|Pr]) :- 
    nth1(T1,S,S1), nth1(T1,D,D1),
    nth1(T2,S,S2),
    S1 + D1 #< S2,
    precedenceConstraints(S,D,Pr).
...
maxExprOfEndSlots([X|L], max(X,E1)) :- 
    maxExprOfEndSlots(L,E1).
