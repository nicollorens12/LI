...
reg(I,J,K,L) :- grid(H,W), cell(I,J), % (I,J,K,L) is a valid region
description
 height(K), width(L),
 I + K - 1 =< H, J + L - 1 =< W.
...
cellInReg(A,B,I,J,K,L) :- % cell (A,B) is inside region
(I,J,K,L)
...
horSymbolInHorRegion :- % every horizontal symbol belongs to a horizontalregion
 % ... %% complete this!
...
expressOr( Var, Lits ) :- symbolicOutput(1), write( Var ), write(' <--> or('),
write(Lits), write(')'), nl, !.
...
main:- current_prolog_flag(os_argv, Argv),
 ...
 main(InputFile), !.
main:- write('Usage: $ ./<executable> <example> or ?- main(<example>).'), nl,
halt.
main(InputFile):-
 symbolicOutput(1), !,
 consult(InputFile),
