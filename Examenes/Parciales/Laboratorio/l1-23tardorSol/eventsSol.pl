...
enforceEachEventExactlyOneDay:- event(E), findall(ed(E,D), day(D), Lits),  exactly(1,Lits), fail.
enforceEachEventExactlyOneDay:- wrongEventForDay(E,D), writeOneClause([ -ed(E,D) ]), fail.
enforceEachEventExactlyOneDay.

enforceEachEventExactlyOneModerator:- event(E),  findall(em(E,M), moderator(M), Lits),  exactly(1,Lits), fail.
enforceEachEventExactlyOneModerator:- wrongEventForModerator(E,M), writeOneClause([ -em(E,M) ]), fail.
enforceEachEventExactlyOneModerator.

enforceRelationEDandEMwithMD:- event(E), day(D), moderator(M),  writeOneClause([ -ed(E,D), -em(E,M), md(M,D) ]), fail.
enforceRelationEDandEMwithMD.

enforceMaxEventsPerDay:- day(D),  maxEventsPerDay(Max), findall(ed(E,D), event(E), Lits),  atMost(Max,Lits), fail.
enforceMaxEventsPerDay.


%% new for this exam:
enforceMaxDaysPerModerator:- moderator(M),  maxDaysPerModerator(Max),  findall(md(M,D), day(D), Lits),  atMost(Max,Lits), fail.
enforceMaxDaysPerModerator.

enforceNotSameDayModerators:- day(D), notSameDayModerators(M1,M2),
                              writeOneClause([ -md(M1,D), -md(M2,D) ]), fail.
enforceNotSameDayModerators.

enforceBefore:- before(E1,E2), day(D1), day(D2), D1 >= D2,  writeOneClause([ -ed(E1,D1), -ed(E2,D2) ]), fail.
enforceBefore.

enforceAllDifferentDaysEvents:- allDifferentDaysEvents(L), member(E1,L), member(E2,L), E1 \= E2, day(D),
...
displaySol(M):-
        maxDaysPerModerator(K), moderator(M1), findall(D, (member(ed(E,D),M), member(em(E,M1),M)), L), sort(L,LS),
...
displaySol(M):- allDifferentDaysEvents(L), member(E1,L), member(E2,L), E1\=E2, 
        ...
        nl,nl,write('Error: incompatible events in the same day: '), write(E1-E2), nl,nl,fail.
displaySol(_).
