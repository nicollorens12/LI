...
oneStep(0,EInicial, EInicial).

oneStep(Cost,EInicial,EFinal):-
    length(EInicial,L),
    between(0,L,Cost),
    between(0,L,I),
    I1 is I+Cost,
    I1 < L,
    swap_elements(EInicial,I,I1,EFinal).


swap_elements(InitialList, I,J, FinalList):-
    nth0(I,InitialList,E1),
    nth0(J,InitialList,E2),
    select(E1,InitialList,Eaux),
    select(E2,Eaux,Ef),
    nth0(I,EInter,E2,Ef),
    nth0(J,FinalList,E1,EInter).
