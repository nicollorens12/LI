...
maxEventsOnADay :-
    day(D),
    findall(ed(E, D), (eventDays(E,Days),member(D,Days)), Lits),
    maxEventsPerDay(MAX), atMost(MAX, Lits), fail.
maxEventsOnADay.

maxDaysModerator :-
    moderator(M),
    findall(md(M,D), day(D), Lits),
    maxDaysPerModerator(MAX), atMost(MAX, Lits), fail.
maxDaysModerator.

varsRelated :-
    moderator(M), day(D), event(E),

    %member(M, LM), member(D, LD),
    % M modera E y E passa en D --> M trabaja en D.
