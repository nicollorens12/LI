...
%% ADDED FOR THIS EXAM:
threeConsecutiveHours(H1,H2,H3) :- hour(H1), H3 is H1+2, hour(H3), H2 is H1+1.
...
twoRestHoursBetweenTasks :-
    ...
    writeOneClause( [-busyAtHour(G,H1), busyAtHour(G,H2), -busyAtHour(G,H3)] ), fail.
twoRestHoursBetweenTasks.

relateDoesVarsWithDoesTask :-
    gangster(G), task(T), findall( does(G,T,H), available(G,H), Lits ),
    expressOr( doesTask(G,T), Lits ), fail.
relateDoesVarsWithDoesTask.

someGangstersAtMostTwoDifferentTasks :-
    maxTwoTaskGangsters(L), member(G,L), findall( doesTask(G,T), task(T), Lits ),
    atMost(2, Lits), fail.
someGangstersAtMostTwoDifferentTasks.
