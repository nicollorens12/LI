...
The following constraints should be met. First of all, the concert
should consist of the specified number of songs. Of course, songs
cannot be repeated. Secondly, to keep the audience's spirits high, the
score of the songs along the concert should never decrease. In the
third place, nobody can sing two songs in a row. Morever, each of the
programme participants has to sing at least one song. Also, there is a
minimum on the number of dance songs and flashlight songs in the
concert. Finally, in a window of consecutive songs of the specified
length, there can be at most one dance song; and similarly for
flashlight songs.
...
writeClauses :-
    ...
    scoreNeverDecreases,            % (*1) the score of songs along the concert never decreases
    noParticipantSings2SongsInARow, % (*2) nobody can sing 2 songs in a row
    everyParticipantAtLeastOneSong, % (*3) every participant sings at least 1 song
    enoughSongsOfEachType,          % (*4) minimum number of dance/flashlight songs is respected
...
scoreNeverDecreases:-
    slot(S1),slot(S2),
    songScore(J1,P1),songScore(J2,P2),
    J1 =\= J2,
    S2 > S1,
    P2 < P1,
    writeOneClause([-x(S1,J1),-x(S2,J2)]),
    fail.
scoreNeverDecreases.
...
noParticipantSings2SongsInARow:-
    song(J1),song(J2),
    songParticipant(J1,P),songParticipant(J2,P),
    slot(S),
    S1 is S+1,
    numberOfSongsForConcert(K),
    S1 =< K,
    writeOneClause([-x(S, J1),-x(S1, J2)]),
    writeOneClause([-x(S, J2),-x(S1, J1)]),
    fail.
noParticipantSings2SongsInARow.
...
everyParticipantAtLeastOneSong:-
    participant(P),
    findall(x(S,J),(slot(S), songParticipant(J,P)),Lits),
    atLeast(1,Lits),
    fail.
everyParticipantAtLeastOneSong.
...
enoughSongsOfEachType:-
    minNumberOfSongsOfType(M, T),
    findall(x(S,J),(slot(S),songType(J,T)),Lits),
    atLeast(M,Lits),
    fail.
enoughSongsOfEachType.
...
atMostOneOfThisTypeInAWindow:-
    ...
    numberOfSongsForConcert(K),
    SlotMax =< K,
    findall(x(S1,J),(songType(J,T),between(S,SlotMax,S1)),Lits),
    atMost(1,Lits),
    fail.
atMostOneOfThisTypeInAWindow.
