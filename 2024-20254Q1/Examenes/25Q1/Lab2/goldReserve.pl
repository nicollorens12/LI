...
% minSubset(K1,L,Sm): Sm is a subset of L with sum > K1 st. if an element is
...
% bc(I,J)   bar I is placed in container J
% c(J)      containers 1, 2,...,J are used
tieBCvarWithCBar(MaxContainers):-
    container(J), J =< MaxContainers, bar(I),
    writeOneClause([-bc(I,J),c(J)]),
    fail.
tieBCvarWithCBar(_).
...
costOfThisSolution(M,Cost) :-
    findall(_,(container(J),member(c(J),M)), L), length(L,Cost).
